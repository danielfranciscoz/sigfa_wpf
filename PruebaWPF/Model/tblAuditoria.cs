//------------------------------------------------------------------------------
// <auto-generated>
//     Este código se generó a partir de una plantilla.
//
//     Los cambios manuales en este archivo pueden causar un comportamiento inesperado de la aplicación.
//     Los cambios manuales en este archivo se sobrescribirán si se regenera el código.
// </auto-generated>
//------------------------------------------------------------------------------

namespace PruebaWPF.Model
{
    using System;
    using System.Collections.Generic;
    
    public partial class tblAuditoria
    {
        public int IdDocumento { get; set; }
        public int IdComprobante { get; set; }
        public byte IdPeriodoEspecifico { get; set; }
        public byte Concecutivo { get; set; }
        public Nullable<byte> IdPrograma { get; set; }
        public Nullable<int> IdImpresion { get; set; }
        public Nullable<System.DateTime> Fecha { get; set; }
        public Nullable<System.DateTime> FechaImpresion { get; set; }
        public string Concepto { get; set; }
        public Nullable<byte> Estado { get; set; }
        public Nullable<byte> IdFuenteFinanciamiento { get; set; }
        public string LoginCreacion { get; set; }
        public Nullable<System.DateTime> FechaCreacion { get; set; }
        public string LoginImpresion { get; set; }
        public Nullable<byte> ValorAnterior_E { get; set; }
        public Nullable<byte> ValorActual_E { get; set; }
        public Nullable<byte> RegAnulado_Anterior { get; set; }
        public Nullable<byte> RegAnulado_Actual { get; set; }
    }
}
