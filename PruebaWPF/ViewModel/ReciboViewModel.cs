...
namespace PruebaWPF.ViewModel
{
    class ReciboViewModel : IGestiones<ReciboSon>
    {
        ...
        public List<ReciboSon> FindAll()
        {
            ...
            if (usuarioCajero)
            {
                recibo = db.Recibo.Where(w => r.Any(a => w.InfoRecibo.IdRecinto == a.IdRecinto) && w.UsuarioCreacion == clsSessionHelper.usuario.Login);

            }
            else
            {
                recibo = db.Recibo.Where(w => r.Any(a => w.InfoRecibo.IdRecinto == a.IdRecinto));
        ...
        private List<ReciboSon> UnirRecibos(IQueryable<Recibo> recibo)
        {
            ...
            List<ReciboSon> ReciboCompleto = (from r in recibo
                        ...
                        ).Take(clsConfiguration.Actual().TopRow).ToList();
